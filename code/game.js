...
//this is the part that you need to change nickson
...
puppy.prototype.type = 'puppy';

function Sora(pos, ch){
	this.pos = pos;
	this.size = new Vector (1,1);
	if (ch == "s"){
		this.speed = new Vector (2,0);
...
DOMDisplay.prototype.drawFrame = function() {
  ...
  //Update the status each time withy this.
...
Sora.prototype.act = function(step,level){
	var newPos = this.pos.plus(this.speed.times(step));
	if (!level.obstacleAt(newPos,this.size))
		this.pos = newPos;
	else if (this.repeatPos)
...
puppy.prototype.act = function(step){
	this.wobble += step * wobbleSpeed;
	var wobblePos = Math.sin(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor){
	...
	if( type == "lava" && this.status == null){
		this.status = "lost";
		this.finishDelay = 1;
	}else if( type == "sora" && this.status == null){
		this.status = "lost";
		this.finishDelay = 1;
	} else if (type == "coin"){
		...
		if (type == "puppy"){
			this.status = alert("the end in near!");
		};
